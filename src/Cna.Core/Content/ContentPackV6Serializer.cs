...
internal static class ContentPackV6Serializer
{
    public static byte[] SerializeCanonical(ContentPackV6Definition definition)
    {
        ...
        var issues = ContentPackV6Validator.Validate(definition).Issues
            .ToArray();
        if (issues.Length > 0)
        {
        ...
        using (var writer = new Utf8JsonWriter(stream))
        {
            ...
            writer.WriteNumber("schemaVersion", ContentPackV6Definition.SchemaVersion);
            writer.WriteString("formatId", ContentPackV6Definition.CanonicalFormatId);
    ...
    public static ContentPackV6ParseResult Deserialize(ReadOnlySpan<byte> utf8Json)
    {
        try
        {
            ...
            {
                return ContentPackV6ParseResult.Failure(
                    "content.noncanonical-json",
                    "Content v6 input must be byte-identical canonical JSON.");
            }

            return ContentPackV6ParseResult.Success(definition);
        }
        catch (ContentPackV6ParseException exception)
        {
            return ContentPackV6ParseResult.Failure(exception.Code, exception.Message);
        }
        catch (JsonException exception)
        {
            return ContentPackV6ParseResult.Failure("content.invalid-json", exception.Message);
        }
        ...
        {
            return ContentPackV6ParseResult.Failure("content.invalid-value", exception.Message);
        }
    }

    private static ContentPackV6Definition ParseDefinition(JsonElement element)
    {
        ...
        if (schemaVersion != ContentPackV6Definition.SchemaVersion)
        {
            Fail(BreakdownCapabilityDiagnostics.Identity, $"Unsupported schema version '{schemaVersion}'.");
        }
        ...
        {
            Fail(BreakdownCapabilityDiagnostics.Identity, $"Unsupported format ID '{formatId}'.");
        }

        if (ReadString(properties["capabilityProfileId"], "/capabilityProfileId")
            != ContentPackV6Definition.SupportedCapabilityProfileId)
        {
            Fail(BreakdownCapabilityDiagnostics.Identity, "Unsupported capability profile.");
        }
        ...
        var combatCapabilityCount = capabilities.Count(value => string.Equals(
            value,
            ContentPackV6Definition.CombatCapabilityId,
            StringComparison.Ordinal));
        if (combatCapabilityCount == 0)
        {
            Fail(
                BreakdownCapabilityDiagnostics.Identity,
                $"Content v6 requires the '{ContentPackV6Definition.CombatCapabilityId}' capability.");
        }
        ...
        if (combatCapabilityCount > 1)
        {
            Fail(
                BreakdownCapabilityDiagnostics.Identity,
                $"Content v6 permits exactly one '{ContentPackV6Definition.CombatCapabilityId}' capability.");
        }
        ...
        var definition = new ContentPackV6Definition(
            legacyResult.Definition!,
            elementFacts,
            placementFacts,
            ReadString(properties["capabilityProfileId"], "/capabilityProfileId"));
        var issues = ContentPackV6Validator.Validate(definition).Issues
            .ToArray();
        if (issues.Length > 0)
        {
            var issue = issues.OrderBy(value => value.Code, StringComparer.Ordinal)
                .ThenBy(value => value.Path, StringComparer.Ordinal).First();
        ...
        List<ContentElementCombatFacts> facts)
    {
        ...
        foreach (var value in ReadArray(element, "/elements"))
        {
            ...
            facts.Add(ContentElementCombatFacts.CreateV6(
                elementId,
                ReadString(
    ...
    private static ContentOrigin ParseOrigin(JsonElement element, string path)
    {
        ...
        var kind = ReadString(properties["kind"], $"{path}/kind") switch
        {
            ...
            var value => throw new ContentPackV6ParseException(
        ...
        params string[] expectedProperties)
    {
        ...
        foreach (var propertyName in expectedProperties)
        {
            ...
            {
                Fail(propertyName == "capabilityProfileId"
                    ? BreakdownCapabilityDiagnostics.Identity
                    : "content.missing-property", $"Missing required property '{propertyName}'.");
            }
        }
    ...
    private static void Fail(string code, string message) =>
        throw new ContentPackV6ParseException(code, message);

    private sealed class ContentPackV6ParseException : Exception
    {
        public ContentPackV6ParseException(string code, string message)
            : base(message)
        {
            Code = code;
        }
