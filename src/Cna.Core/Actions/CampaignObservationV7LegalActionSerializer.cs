...
/// <summary>
/// Strict current Observation 7 action-set codec.
/// </summary>
internal static class CampaignObservationV7LegalActionSerializer
{
    public static byte[] Serialize(CampaignLegalActionSet actionSet)
    {
        ArgumentNullException.ThrowIfNull(actionSet);
        if (actionSet.PolicyId != CampaignLegalActionSet.CurrentPolicyId
            || !Cna1979Ruleset.IsCanonicalHash(actionSet.RulesetHash))
            throw new ArgumentException("Only current action sets may use the Observation 7 codec.", nameof(actionSet));
    ...
    public static CampaignLegalActionSet DeserializeCanonical(ReadOnlySpan<byte> canonicalJson)
    {
        try
        {
                ...
                || !canonicalJson.SequenceEqual(Serialize(result)))
            {
                throw new JsonException(
                    "The Observation 7 legal-action set is not canonical JSON.");
            }
        ...
        {
            throw new JsonException(
                "The Observation 7 legal-action set JSON is invalid.",
                exception);
        }
    }
        ...
        CampaignObservationV7 observation)
    {
        ...
        var expected = parsed.Audience == CampaignActionAudience.System
            ? CampaignObservationV7ActionDerivation.DeriveSystem(observation)
            : CampaignObservationV7ActionDerivation.DerivePlayer(observation);
        if (parsed != expected)
        {
            throw new JsonException(
                "The Observation 7 legal-action set is not the exact current membership.");
        }
    ...
    private static void WriteCandidate(Utf8JsonWriter writer, CampaignActionCandidate candidate)
    {
        if (candidate is not (MoveElementAction or CompleteMovementSegmentAction
            ...
            or StopElementMovementAction or ResolveBreakdownStopAction or CompleteBreakdownSegmentAction))
        {
        ...
        switch (candidate)
        {
            ...
            case StopElementMovementAction stop:
                writer.WriteString("routeId", stop.RouteId);
                break;
            case ResolveBreakdownStopAction resolve:
                writer.WriteString("stopId", resolve.StopId);
                break;
            case CompleteBreakdownSegmentAction complete:
                writer.WriteNumber("operationStage", complete.OperationStage!.Value);
                break;
            default:
    ...
    private static CampaignActionCandidate ParseCandidate(JsonElement candidate)
    {
        ...
        CampaignActionCandidate parsed = kind switch
        {
            ...
            "stop-element-movement" => ParseHandle(candidate, "routeId", value => new StopElementMovementAction(value)),
            "resolve-breakdown-stop" => ParseHandle(candidate, "stopId", value => new ResolveBreakdownStopAction(value)),
            "complete-breakdown-segment" => ParseBreakdownCompletion(candidate),
    ...
    private static CampaignActionCandidate ParseHandle(JsonElement candidate, string property,
        Func<string, CampaignActionCandidate> create)
    {
        CampaignLegalActionSerializer.RequireProperties(candidate, "contractVersion", "actionId", "kind", property);
        return create(candidate.GetProperty(property).GetString()!);
    }

    private static CompleteBreakdownSegmentAction ParseBreakdownCompletion(JsonElement candidate)
    {
        CampaignLegalActionSerializer.RequireProperties(candidate, "contractVersion", "actionId", "kind", "operationStage");
        return new CompleteBreakdownSegmentAction(candidate.GetProperty("operationStage").GetInt32());
    }
