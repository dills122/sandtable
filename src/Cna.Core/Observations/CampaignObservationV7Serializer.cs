...
internal static class CampaignObservationV7Serializer
{
    public static byte[] SerializeCanonical(CampaignObservationV7 observation)
    {
        ...
        using (var writer = new Utf8JsonWriter(stream))
        {
            ...
            writer.WriteString("capabilityProfileId", observation.CapabilityProfileId);
            writer.WriteString(
                "observer",
            ...
            writer.WriteStartArray("ownBrokenVehicleLots");
            foreach (var lot in observation.OwnBrokenVehicleLots)
            {
                writer.WriteStartObject();
                writer.WriteString("cohortId", lot.CohortId);
                writer.WriteString("locationId", lot.LocationId);
                writer.WriteNumber("pointCount", lot.PointCount);
    ...
    public static CampaignObservationV7 DeserializeCanonical(ReadOnlySpan<byte> canonicalJson)
    {
        try
        {
            ...
            CampaignObservationSerializer.RequireProperties(
                ...
                "decisionState",
                "ownBrokenVehicleLots");
            var result = new CampaignObservationV7(
                ...
                root.GetProperty("ownBrokenVehicleLots").EnumerateArray().Select(ParseLot).ToArray());
            if (!canonicalJson.SequenceEqual(SerializeCanonical(result)))
            {
                throw new JsonException("The Campaign Observation v7 is not canonical JSON.");
            }
        ...
        {
            throw new JsonException("The Campaign Observation v7 JSON is invalid.", exception);
        }
    }
        ...
        CampaignObservationV7DecisionState decisionState)
    {
        ...
        {
            case CampaignObservationV7NormalDecisionState normal:
                writer.WriteString("kind", "normal");
                if (normal.ActiveMovement is null) writer.WriteNull("activeMovement");
                else
                {
                    writer.WriteStartObject("activeMovement");
                    writer.WriteString("routeId", normal.ActiveMovement.RouteId);
                    writer.WriteString("elementId", normal.ActiveMovement.ElementId);
                    writer.WriteString("originLocationId", normal.ActiveMovement.OriginLocationId);
                    writer.WriteString("currentLocationId", normal.ActiveMovement.CurrentLocationId);
                    writer.WriteEndObject();
                }
                break;
            case CampaignObservationV7BreakdownWaitingDecisionState:
                writer.WriteString("kind", "breakdown-waiting");
                break;
            case CampaignObservationV7PhasingWaitingDecisionState waiting:
                writer.WriteString("kind", "phasing-waiting");
                writer.WriteString("windowId", waiting.WindowId);
                break;
            case CampaignObservationV7ReactingDecisionState reacting:
                ...
                break;
            default:
                throw new JsonException("Unknown Campaign Observation v7 decision state.");
        }

        writer.WriteEndObject();
    }

    internal static CampaignObservationV7DecisionState ParseDecisionState(JsonElement state)
    {
        var kind = state.GetProperty("kind").GetString();
        return kind switch
        {
            "normal" => ParseNormal(state),
            "breakdown-waiting" => ParseBreakdownWaiting(state),
            "phasing-waiting" => ParsePhasing(state),
            "reacting" => ParseReacting(state),
            _ => throw new JsonException(
                $"Unknown Campaign Observation v7 decision kind '{kind}'."),
        };
    }

    private static CampaignObservationV7NormalDecisionState ParseNormal(JsonElement state)
    {
        CampaignObservationSerializer.RequireProperties(state, "kind", "activeMovement");
        var route = state.GetProperty("activeMovement");
        if (route.ValueKind == JsonValueKind.Null) return new CampaignObservationV7NormalDecisionState();
        CampaignObservationSerializer.RequireProperties(route, "routeId", "elementId", "originLocationId", "currentLocationId");
        return new CampaignObservationV7NormalDecisionState(new ObservedMovementRoute(
            route.GetProperty("routeId").GetString()!, route.GetProperty("elementId").GetString()!,
            route.GetProperty("originLocationId").GetString()!, route.GetProperty("currentLocationId").GetString()!));
    }

    private static CampaignObservationV7BreakdownWaitingDecisionState ParseBreakdownWaiting(JsonElement state)
    {
        CampaignObservationSerializer.RequireProperties(state, "kind");
        return new CampaignObservationV7BreakdownWaitingDecisionState();
    }

    private static ObservedBrokenVehicleLot ParseLot(JsonElement lot)
    {
        CampaignObservationSerializer.RequireProperties(lot, "cohortId", "locationId", "pointCount");
        return new ObservedBrokenVehicleLot(lot.GetProperty("cohortId").GetString()!,
            lot.GetProperty("locationId").GetString()!, lot.GetProperty("pointCount").GetInt32());
    }

    private static CampaignObservationV7PhasingWaitingDecisionState ParsePhasing(JsonElement state)
    {
        CampaignObservationSerializer.RequireProperties(state, "kind", "windowId");
        return new CampaignObservationV7PhasingWaitingDecisionState(
            state.GetProperty("windowId").GetString()!);
    }

    private static CampaignObservationV7ReactingDecisionState ParseReacting(JsonElement state)
    {
        CampaignObservationSerializer.RequireProperties(
            state,
            "kind",
        ...
        return new CampaignObservationV7ReactingDecisionState(
            state.GetProperty("windowId").GetString()!,
