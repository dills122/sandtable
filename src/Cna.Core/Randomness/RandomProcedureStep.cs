...
public sealed record RandomProcedureCondition
{
    ...
    public bool Equals(RandomProcedureCondition? other) =>
        ReferenceEquals(this, other)
        || (other is not null
            && string.Equals(Label, other.Label, StringComparison.Ordinal)
            && WhenKindIn.SequenceEqual(other.WhenKindIn, StringComparer.Ordinal));

    public override int GetHashCode()
    {
        var hash = new HashCode();
        hash.Add(Label, StringComparer.Ordinal);
        foreach (var kind in WhenKindIn)
        {
...
public sealed record RandomProcedureStep
{
    ...
    public bool Equals(RandomProcedureStep? other) =>
        ReferenceEquals(this, other)
        || (other is not null
            && string.Equals(ProcedureId, other.ProcedureId, StringComparison.Ordinal)
            && AcceptedD6Order.SequenceEqual(other.AcceptedD6Order, StringComparer.Ordinal)
            && string.Equals(Repeat, other.Repeat, StringComparison.Ordinal)
            && ConditionalAcceptedD6 == other.ConditionalAcceptedD6);

    public override int GetHashCode()
    {
        var hash = new HashCode();
        hash.Add(ProcedureId, StringComparer.Ordinal);
        foreach (var label in AcceptedD6Order)
        {
            hash.Add(label, StringComparer.Ordinal);
        }
        hash.Add(Repeat, StringComparer.Ordinal);
        hash.Add(ConditionalAcceptedD6);
        return hash.ToHashCode();
    }
}
