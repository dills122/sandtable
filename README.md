...
| Combat and continual cycle | Checkpoint B accepted; dormant Tasks005–007 and Task008 creation codecs implemented; gameplay dormant | 28 creation-rooted traces and Task004 acceptance map all 72 requirements. Selected Rules, Content7 and World7 state are implemented; initial World7, Rules10, Setup7, configuration, creation request/Created11 and creation-only Snapshot12 codecs have exact C# parity; A1c/A2 are reviewed on stacked feature branches; B1 opening replay is reviewed in PR126; C Weather replay is reviewed in PR127; B2 stage-entry replay is implemented through Reserve entry. Task008 parent restore and later gameplay, public activation and simulator gates remain open. |
...
1. Continue Task008's bounded Combat persistence children: initial World7 readback, dormant Rules10 manifest, and Setup7/configuration codecs are merged; A1c creation request/Created11 is reviewed in PR124. A2 creation-only Snapshot12 is reviewed in PR125. B1 opening replay is reviewed in PR126; C Weather replay is reviewed in PR127. B2 stage-entry replay is implemented; Reserve and remaining inherited families follow. Actual publication proof remains open under HOST-PUB-001. Dormant Tasks005–007 Rules, Content and World foundations are implemented.
