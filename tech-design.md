...
[technical design](docs/design/zoc-reaction-v1.md). `ZOR-TASK-002A`-`003A` complete the dormant
Rules/Content/fixture gate and add direct-only World 5/creation 9 state plus current-raw,
Movement-ended/reacting-position, and Reaction-identity seams; ruleset 7, Content schema 4, World 4,
and CampaignCreated 8 remain active until the coordinated migration. `ZOR-TASK-003B` is next.
...
`ZOR-TASK-002A`-`003A` implement dormant Rules/Content/fixture and Campaign World/creation seams;
`003B` and later slices remain dependency-gated.
