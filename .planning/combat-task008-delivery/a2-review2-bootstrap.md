...
Review instance: **2 of 3** for A2. One final fourth round only if blockers remain after round3 and
...
`dotnet test --project tests/Cna.Core.Tests/Cna.Core.Tests.csproj --no-build --filter-class '*CombatCreationSnapshotTests' '-bl:/tmp/a2-review2-{}.binlog'`

`PYTHONDONTWRITEBYTECODE=1 python3 docs/specs/verify-combat-authority-envelope-v1.py`

`git diff --check`

Read-only except own `a2-review2-report.md` in planning directory. Do not modify implementation/docs,
