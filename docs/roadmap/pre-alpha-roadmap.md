...
Combat contract packets now compose selected creation-rooted histories through released-I armed
continuation and Movement completion, direct Reaction closure, active-participant System fallback,
and post-second-move participant completion/closure (`003D2c.3q`), but they do not activate Combat
or Reserve Release in Core or the simulator. Historical
review and simulator reports apply only to their stated commits and capabilities.

| Next gate | Concrete outcome |
| --- | --- |
| Contract checkpoint B | D2c.3 selected infantry/Reserve-I profile is frozen through `.3q`; complete D2c.4 full World/Snapshot composition and the003 handoff, then Task004A side projection,004B Exercise/Runner evidence and004C's complete acceptance-criteria map. Multiple-opportunity Reaction and positive vehicle Breakdown are post-B profile extensions, not blockers. Accept exact versions, capacity, compatibility and unsupported-state behavior. |
...
| Combat and continual cycle | Research/design complete; selected D2c.3 profile contract-frozen through post-second-move Reaction completion (`003D2c.3q`) | **Not implemented or activated**; current public authority stops at Combat entry | D2c.4 → Task004A/B/C → checkpoint B → runtime005–021 → evidence022–024 |
...
| 5 — Combat and continual-cycle loop | Contract freeze in progress; no Combat runtime | `003D2c.3q` closes selected-profile D2c.3. Finish `.4`, Task004A/B/C and checkpoint B; then execute005–024 and close025 only after an authentic loop passes |
...
authority. Guarded repeat and positive released-Reserve Movement/expiry remain open at this
checkpoint; broader Reaction/vehicle families are post-B extensions and D2c.4 remains open.
...
Movement/exception expiry remains open at this checkpoint; broader Reaction/vehicle families are
post-B extensions and D2c.4 remains open.
...
completion/closure. That closes D2c.3 for the selected infantry/Reserve-I profile; full
World/Snapshot composition remains required before parent003/checkpoint B closes.

Movement, bounded ZOC/Reaction, and Breakdown are complete within their certified runtime profiles.
Combat contract work is complete through `003D2c.3q`: creation-rooted ordinary Movement, stop and
...
runtime. Multiple-opportunity Reaction and positive vehicle Breakdown are explicit post-B profile
extensions. D2c.4 composition, Task004A/B/C, and checkpoint B remain open. Completed
...
| --- | --- |
| Combat critical path | D2c.4 integrated composition and003 handoff → Task004A/B/C → checkpoint B → dormant005–019 → public020–021 → Runner022–024 → closeout025. |
...
| `COMBAT-CYCLE-001` | Source inventory and TASK-001/002 source/Content packets complete; checkpoint A author check and003A/003B/003C1/003D1/003C2/003C3a contract slices complete (C2 creation cut, C3a bounded control); 003C3b/c bounded contract checkpoints complete; parent003C/D and remaining combined contracts open | Replace provisional Sprint 5 headings with reviewed private-choice, simultaneous-resolution, Reserve Release, and repeat-cycle contracts |
...
POL-001–008 and the corrected combined plan at `a10a588`. Contract evidence now closes the selected
D2c.3 infantry/Reserve-I profile through Reaction completion after second movement at
`003D2c.3q`; D2c.4, Task004A/B/C, and checkpoint B stay open. Multiple-opportunity Reaction and
positive vehicle Breakdown are post-B extensions. The
...
Reaction completion after second movement through `003D2c.3q`; the combined plan's current status
table is authoritative for that detailed chain. Parent003D2c/003, Task004A/B/C, checkpoint B, and all
