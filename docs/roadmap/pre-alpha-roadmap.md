...
| --- | --- |
| Contract checkpoint B | Freeze actual ordinal-2 Movement completion/exception expiry and remaining bounded D2c.3 capability families, then D2c.4 full World/Snapshot composition and Task004's side/Exercise contract plus complete acceptance-criteria map. Accept exact versions, capacity, compatibility, unsupported-state behavior, and the 003-to-004 handoff. |
...
| Combat and continual cycle | Research/design complete; contract packets through released-I Movement (`003D2c.3l`) | **Not implemented or activated**; current public authority stops at Combat entry | Movement completion/exception expiry and remaining D2c.3 families → D2c.4 → Task004 → checkpoint B → runtime005–021 → evidence022–024 |
...
| 5 — Combat and continual-cycle loop | Contract freeze in progress; no Combat runtime | `003D2c.3l` is the latest completed child. Freeze Movement completion/exception expiry, finish remaining `.3`, `.4`, Task004 and checkpoint B; then execute005–024 and close025 only after an authentic loop passes |
...
retaining armed resources and the pending exception. Actual Movement completion/expiry remains open.
...
cuts,1748 mutations and700 raw rejects. D2c must authenticate actual Movement completion/exception
expiry and supply full World/Snapshot composition before parent003/checkpoint B closes.

Movement, bounded ZOC/Reaction, and Breakdown are complete within their certified runtime profiles.
Combat contract work is complete through `003D2c.3l`: creation-rooted ordinary Movement, stop and
...
released-I ordinal-2 move. This is executable
contract evidence, not Combat
runtime. Movement completion/exception expiry, broader Reaction and vehicle
...
released-I ordinal-2 Movement through `003D2c.3l`; Movement completion/expiry, remaining
D2c.3 families, D2c.4,
...
released-I ordinal-2 Movement through `003D2c.3l`; the combined plan's current status
