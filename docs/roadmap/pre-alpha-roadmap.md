...
continuation, Movement completion, and active-participant System fallback, but they do not activate Combat or
Reserve Release in Core or the simulator. Historical
review and simulator reports apply only to their stated commits and capabilities.

| Next gate | Concrete outcome |
| --- | --- |
| Contract checkpoint B | Freeze remaining bounded D2c.3 capability families, then D2c.4 full World/Snapshot composition and Task004's side/Exercise contract plus complete acceptance-criteria map. Accept exact versions, capacity, compatibility, unsupported-state behavior, and the 003-to-004 handoff. |
...
| Combat and continual cycle | Research/design complete; contract packets through post-second-move Reaction completion (`003D2c.3q`) | **Not implemented or activated**; current public authority stops at Combat entry | Remaining D2c.3 families → D2c.4 → Task004 → checkpoint B → runtime005–021 → evidence022–024 |
...
| 5 — Combat and continual-cycle loop | Contract freeze in progress; no Combat runtime | `003D2c.3q` is the latest completed child. Finish remaining `.3`, `.4`, Task004 and checkpoint B; then execute005–024 and close025 only after an authentic loop passes |
...
families, and D2c.4 composition remain open.
...
Movement/exception expiry, broader Reaction and vehicle families, and D2c.4 composition remain open.
...
completion/closure; it must still
...
runtime. Multiple-opportunity Reaction and vehicle
...
| --- | --- |
| Combat critical path | Remaining D2c.3 children → D2c.4 integrated composition and003 handoff → Task004/Checkpoint B → dormant005–019 → public020–021 → Runner022–024 → closeout025. |
...
Reaction completion after second movement through `003D2c.3q`; remaining D2c.3 families, D2c.4,
