...
| Movement and contact | Movement Foundation is complete through merged `MOV-TASK-010` / PR #79; dormant ZOC/Reaction Rules and Content successors are implemented through `ZOR-TASK-002B` | Add the bounded fixture/readback evidence in `002C`; Breakdown adjudication remains a separate gate |
...
| 4 — Movement, Breakdown, and Reaction boundary | Active | Movement Tasks 001-010 are complete and merged; approved dormant ZOC/Reaction checkpoints are implemented through `ZOR-TASK-002B`, while Breakdown adjudication remains separate |
...
[technical design](../design/zoc-reaction-v1.md); dormant checkpoints `ZOR-TASK-002A`-`002B` are
implemented and `002C` is next.
...
the approved ZOC/Reaction package, implemented through `ZOR-TASK-002B`. Its Rules, Content, Campaign,
...
| --- | --- | --- |
| ZOC/Reaction authority lane | Stable approved requirements, technical design, task slices, and traceability from accepted `CONTACT-001` rulings | Dormant checkpoints `ZOR-TASK-002A`-`002B` are implemented; `002C` is next; approval did not collapse the dependency graph |
...
package. Its dependency-ordered dormant implementation is complete through `ZOR-TASK-002B`.
...
| `CONTACT-001` | Decision-complete; all five rulings accepted in PR #71; ZOC/Reaction spec/design approved after PR #80 merged | Dormant implementation is complete through `ZOR-TASK-002B`; Contact/Engaged remain Sprint 5 |
...
[technical design](../design/zoc-reaction-v1.md) are approved; `ZOR-TASK-002A`-`002B` are implemented
as dormant Rules/Content slices and leave ruleset 7 / Content schema 4 active.
...
is complete through `002B`, and `002C` is the next bounded checkpoint.
