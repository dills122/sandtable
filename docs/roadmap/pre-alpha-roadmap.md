...
| Movement and contact | Movement Foundation is complete through merged `MOV-TASK-010` / PR #79; the dormant ZOC/Reaction Rules/Content/fixture gate is implemented through `ZOR-TASK-002C` | Add dormant Campaign state and creation successors in `003A`; Breakdown adjudication remains a separate gate |
...
| 4 — Movement, Breakdown, and Reaction boundary | Active | Movement Tasks 001-010 are complete and merged; approved dormant ZOC/Reaction checkpoints are implemented through `ZOR-TASK-002C` and `003A` is next, while Breakdown adjudication remains separate |
...
[technical design](../design/zoc-reaction-v1.md); dormant checkpoints `ZOR-TASK-002A`-`002C` are
implemented and `003A` is next.
...
the approved ZOC/Reaction package, implemented through `ZOR-TASK-002C`. Its Rules, Content, Campaign,
...
| --- | --- | --- |
| ZOC/Reaction authority lane | Stable approved requirements, technical design, task slices, and traceability from accepted `CONTACT-001` rulings | Dormant checkpoints `ZOR-TASK-002A`-`002C` are implemented; `003A` is next; approval did not collapse the dependency graph |
...
package. Its dependency-ordered dormant implementation is complete through `ZOR-TASK-002C`.
...
| `CONTACT-001` | Decision-complete; all five rulings accepted in PR #71; ZOC/Reaction spec/design approved after PR #80 merged | Dormant implementation is complete through `ZOR-TASK-002C`; `003A` is next; Contact/Engaged remain Sprint 5 |
...
[technical design](../design/zoc-reaction-v1.md) are approved; `ZOR-TASK-002A`-`002C` are implemented
as the dormant Rules/Content/fixture gate and leave ruleset 7 / Content schema 4 active. `003A` is
next.
...
is complete through `002C`, and `003A` is the next bounded checkpoint.
