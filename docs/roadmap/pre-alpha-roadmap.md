...
## Status model and document ownership

Sandtable uses four distinct delivery states. A narrower state never implies a later one:

| State | Meaning | Evidence required |
| --- | --- | --- |
| Researched | Sources, ambiguities, and a bounded direction are recorded | Source inventory, ruling or decision record, and explicit unknowns |
| Contract-frozen | Versioned shapes, invariants, transitions, and negative cases are executable as retained contract evidence | Spec/schema/fixtures plus failure-sensitive oracle; no production capability claim |
| Implemented | The Umpire can execute and replay the capability through typed runtime code | Focused C# tests, compatibility/recovery checks, and build/format evidence |
| Activated and witnessed | Side-safe public actions and strict Exercise/Runner evidence use the capability end to end | Disclosure negatives, admission tests, reconstruction/readjudication, and retained deterministic runs |

This roadmap is the canonical source for current capability and cross-package sequencing. The
[Combat plan](../design/combat-cycle-implementation-plan.md) owns Combat task dependencies and
checkpoint evidence. `README.md` summarizes the executable product; `tech-design.md` owns stable
architecture; `naming-overview.md` owns vocabulary. Research and review files are dated evidence,
not live status ledgers. When those files disagree, correct this roadmap first and update summaries
in the same change.

## Current checkpoint and next gates

[Author doc/planning review](../reviews/combat-delivery-plan-author-review.md) records the accepted
planning direction. The executable engine remains Rules9 through first-side Combat **entry**.
Parent003 Combat authority contracts now compose 28 selected creation-rooted histories through
released-I armed continuation, Movement completion, active-participant System fallback and guarded
repeat/finish. They do not activate Combat or Reserve Release in Core or simulator. Historical
review and simulator reports apply only to their stated commits and capabilities.
...
| Dormant Core,005–019 | Tasks005–007 and Task008 A0/A1a/A1b are implemented and merged. A1c creation request/Created11 is reviewed in PR124; A2 creation-only Snapshot12 is reviewed in PR125. B1 opening replay is reviewed in PR126; C Weather replay is reviewed in PR127. B2 stage-entry is reviewed in PR128. D1 own Reserve designation is reviewed in PR129; D2 completion codecs are reviewed in PR130. Task019A first opening is reviewed in PR131. E1 ordinary Move4 is reviewed in PR132; joint E2/G1 route lifecycle is reviewed in PR133; G2 Breakdown completion is reviewed in PR134; F1 Reaction trigger is reviewed in PR135; F2 participant lifecycle is implemented and reviewed in PR136 (focused16/full2,060; three independent rounds); F3 direct closure is implemented and reviewed in PR136 (focused25/full2,069; three independent rounds); F4 active fallback is implemented and reviewed in PR136 (focused29/full2,080; three independent rounds). F5 second reactor move is implemented and reviewed in PR136 (focused22/full2,093; three independent rounds). F6 completion is implemented and reviewed in PR136 (focused24/full2,104; three independent rounds). [Initial H](../design/combat-cycle-implementation-plan.md#task008-execution-index) is accepted in PR136:368 exact roots/286 histories, full2,244/boundary81, dev review, three independent rounds and exact-head CI. Core codec/replay gate is complete; Task008 actual-publication evidence remains open. Task009A inherited assessment/bindings is accepted (focused10/full2,254/boundary81; dev review, three ordinary rounds and one conditional isolated final review, exact-head CI);009B positive certification is accepted (full2,258/boundary81; dev review, three independent rounds and exact-head CI). Task009 dormant mechanisms are complete;010A actual empty selection/traversal is accepted (full2,265/boundary81; dev review, three independent rounds and exact-head CI). Timed selection010B is accepted (full2,277/boundary81; dev review, three independent rounds and exact-head CI). Cumulative routing010C is accepted (full2,284/boundary81; dev review, three independent rounds and exact-head CI). Parent010 dormant gate is complete; private seals/Prepared011 is accepted (full2,292/boundary81; dev review, three independent rounds and exact-head CI). Atomic commitment012 is accepted (full2,301/boundary81; dev review, three independent rounds and exact-head CI). Result/cursor013 is accepted (full2,311/boundary81; dev review, three independent rounds and exact-head CI). Loss/retreat014 is accepted (full2,318/boundary81; dev review, three independent rounds and exact-head CI). Custody015 implementation is under verification; relationship and round closure016 follows its review gate. Private decisions, settlement, Reserve Release, relation-aware Movement and guarded repeat/finish follow their dependencies. Every state-changing family needs replay cuts and recovery evidence before dependent activation. |
...
DecisionWorker registers its client but does not dispatch campaign decisions. The earlier planning
direction after `b8be39a` has been executed through checkpoint B and the bounded Orleans probe;
the inherited-path handoff and progressive Core evidence remain implementation gates.
The [creation publication evidence gate `HOST-PUB-001`](../specs/combat-authority-envelope-v1.md#runtime-evidence-ownership)
retains Task008's actual-persistence obligation after020–021 and one verified023 trace. A2/H can
close their Core codec/replay scope while publication evidence stays open; production hosting and
durable save/resume cannot inherit a passing claim from memory CAS or creation-only readback.

### Bounded hosting investigation — HOST-RSH-001

**Status:** [Research complete](../research/orleans-publication-feasibility.md) at input `6852088`;
...
three primary files. The investigation remains independent of Combat checkpoint B; current Combat
status is recorded above rather than duplicated in this hosting history.
...
production provider is selected. The experiment closed with the linked decision-ready report,
including failure outcomes and unexercised cases; it does not close a durable-host acceptance gate.
...
| Capability area | Contract evidence | Executable today | Next gate |
| --- | --- | --- | --- |
...
| Combat and continual cycle | Checkpoint B accepted; dormant foundations and Initial H Core codec/replay gate complete;008 publication open | **Gameplay not activated**; current public authority stops at Combat entry | Dormant009A/009B →010–019 → public020–021 → evidence022–024 |
...
| Exercise Harness | Evidence contracts and current Movement/Reaction/Breakdown profiles established | Single Exercise, serial-unpaired Maneuver, controller matrices, and bounded paired comparisons | Add Combat public actions/terminals only after Task021 |

The roadmap deliberately distinguishes foundation maturity from playable breadth. The architecture
is substantially established. Bounded Movement, Reaction and Breakdown authority now reaches
...
records1,670 tests and81 boundary cases. The campaign is not yet playable: Contact/Engaged, Combat
adjudication, published scenario content, remaining Land rules, victory, campaign persistence and
Maproom remain future work.
...
| 5 — Combat and continual-cycle loop | Checkpoint B accepted; dormant005–007 and 008A0/A1a/A1b implemented; no Combat gameplay | Complete008, then009–024; close025 only after an authentic loop passes |
...
[technical design](../design/zoc-reaction-v1.md); checkpoints `ZOR-TASK-002A`-`006C` are
implemented; `007A` adds bounded Runner adoption and `007B` completes strict evidence/review.
...
### Retained contract execution checkpoint

The following notes and lane table record the pre-runtime contract checkpoint. The [current
checkpoint](#current-checkpoint-and-next-gates) and [current delivery status](#current-delivery-status)
supersede their then-next statements.

[D2c.1](../specs/combat-inherited-successors-v1.md) freezes20 inherited successor declarations
and isolated first opening:4 traces/8 cuts,216 mutations/32 raw rejects/88 boundary checks.
[D2c.2a](../specs/combat-opening-preamble-v1.md) now reaches Weather entry from validated
Created11:6traces/30cuts,1050event and3348state mutations. [D2c.2b Weather](../specs/combat-weather-v1.md)
now reaches Organization entry with34traces/68cuts. [D2c.2c stage entry](../specs/combat-stage-entry-v1.md)
reaches Reserve entry with12traces/60cuts. [D2c.2d Reserve designation/completion](../specs/combat-reserve-designation-v1.md)
composes actual first opening for both acting sides and empty/I selection.
[Movement preparation](../design/combat-inherited-movement-preparation.md) maps legacy fields and
provenance requirements. [D2c.3a inherited Movement](../specs/combat-inherited-movement-v1.md)
now supplies real ordinary Move4 history through CP14/DP4 for both sides.
[D2c.3b route lifecycle](../specs/combat-inherited-movement-lifecycle-v1.md) now derives owner stop2,
...
admits that moved state and closes zero-candidate selection in two System events.
[D2c.3e no-attack traversal](../specs/combat-inherited-no-attack-v1.md) adds six System structural
completions to same-slot Reserve Release;
[D2c.3f Reaction trigger](../specs/combat-inherited-reaction-trigger-v1.md) replays one actual Move4
prefix for each owner, freezes one eligible opposing opportunity and enters Reaction. Participant
lifecycle continues through [D2c.3g](../specs/combat-inherited-reaction-lifecycle-v1.md): one reactor
move, explicit completion, required empty-stop resolution and no-eligible closure resume phasing.
[D2c.3n direct Reaction closure](../specs/combat-inherited-reaction-closure-v1.md) instead adds owner
decline and distinct System unavailable/timeout exits from each exact trigger, closing the sole
unresolved opportunity and resuming phasing without material effects.
...
[D2c.3q Reaction movement completion](../specs/combat-inherited-reaction-movement-completion-v1.md)
then completes that exact participant, resolves its mandatory empty stop, closes the exhausted
window, and resumes the suspended phasing route at authority18.
...
exception. [D2c.3j armed continuation](../specs/combat-inherited-armed-continuation-v1.md) proves
one exact ammunition10 next-cycle candidate per owner and pins full-result support without emitting
authority. Guarded repeat, positive released-Reserve Movement/expiry, broader Reaction and vehicle
families remained open at that child boundary; later children and D2c.4 close selected composition.

[D2c.3k guarded cycle control](../specs/combat-inherited-cycle-control-v1.md) now composes those exact
proofs with inherited Release state. Four System-open/owner-close traces prove repeat into ordinal-2
Movement or finish into same-slot Truck Convoy for both owners, with deterministic deadline,
clock-loss, controller-unavailable and overflow handling. Positive ordinal-2 released-Reserve
Movement/exception expiry and D2c.4 were still follow-ons at that child boundary; broader Reaction
and vehicle families remain extensions.

[D2c.3l released-I Movement](../specs/combat-inherited-reserve-movement-v1.md) consumes each exact
repeat terminal and records one Clear move from assault to own rear at CP0→2 under ceiling10 while
retaining armed resources and the pending exception.

[D2c.3m released-I Movement completion](../specs/combat-inherited-reserve-movement-completion-v1.md)
replays those exact terminals through deliberate stop, empty resolution and accepted completion at
authority31. Its ordinal-2 proof covers current World locations, and unchanged D2b.2 expiry binds
the released member's exception to the actual completion receipt.

[D2c.3n direct Reaction closure](../specs/combat-inherited-reaction-closure-v1.md) replays both exact
3f terminals through six one-event forks: reacting-owner decline plus System unavailable/timeout.
All close the sole unresolved opportunity and restore the exact suspended phasing route with
unchanged World/RNG/progress; Core does not own timeout scheduling.

[D2c.3o active Reaction fallback](../specs/combat-inherited-reaction-active-fallback-v1.md) replays
both exact 3g post-first-move states through four two-event forks. System unavailable/timeout closes
...
[D2c.3q Reaction movement completion](../specs/combat-inherited-reaction-movement-completion-v1.md)
replays both exact3p terminals through owner completion, System empty-stop resolution and System
no-eligible closure. Supply/CP4 World and the three-location reactor track remain exact while the
original suspended phasing route resumes.

[D2c.4 authority composition](../specs/combat-authority-composition-v1.md) integrates 28 selected
creation-rooted traces, full World/Snapshot/history/obligation projections, capacity and exact
CON-002–004 versions. Its Task004 handoff closes parent003 and leaves Task004/checkpoint B open.
...
exhausted; review14 is not a Ready verdict for3d/3e. [Review15](../reviews/combat-inherited-reserve-release-review-15.md) later assessed3i as Ready with one
non-blocking retained-coverage finding, corrected before `719ea0d`; the explicitly extended15of15
sequence is now exhausted. Unreviewed intervening children retain author verification only.
The
[D2b.2 cycle control](../specs/combat-cycle-control-v1.md) has author evidence:19 cases/64 traces,164
cuts,1748 mutations and700 raw rejects. D2c authenticates actual ordinal-2 Movement completion,
exception expiry, active Reaction fallback, and active participant second movement through explicit
completion/closure. [D2c.4 authority composition](../specs/combat-authority-composition-v1.md)
supplies full World/Snapshot composition and exact Task004 handoff, closing parent003 while
checkpoint B remains open for Task004.

Movement, bounded ZOC/Reaction, and Breakdown are complete within their certified runtime profiles.
Combat inherited contract work through `003D2c.3q` includes creation-rooted ordinary Movement, stop and
...
participant step and its explicit completion/closure. This is executable
contract evidence, not Combat runtime. D2c.4 composes 28 selected traces, reconciles CON-002–004
and closes parent003. Multiple-opportunity Reaction, vehicle families, Task004, and checkpoint B
remain open. Completed
...
| --- | --- |
| Combat critical path | Checkpoint B/Task005 complete → dormant006–019 → public020–021 → Runner022–024 → closeout025. Parent003 integrated composition and handoff are complete. |
...
package. Its dependency-ordered Core implementation is complete through `ZOR-TASK-006C`.
...
| `COMBAT-CYCLE-001` | Source inventory, TASK-001/002 and parent003 CON-002–004 authority freeze complete; D2c.4 reconciles 28 creation-rooted traces and supplies exact Task004 handoff | Task004 freezes CON-005/006 and expands all 72 ACs before checkpoint B and dormant runtime |
...
[technical design](../design/zoc-reaction-v1.md) are approved; `ZOR-TASK-002A`-`006C` implement and
activate Ruleset 8, Land sequence 3, Content 5, World 5, CampaignCreated 9, Snapshot 10,
`ElementMoved` 2, Observation 6/policy/history, topology-local action/readback/mapping, exact
Reaction participant/closure authority, current replay, and the user-space manifest/transcript
boundary. `007A` Runner adoption is implemented; `007B` verification and independent review are complete.
...
codec/replay gate is accepted in PR136; Task009A inherited assessment/bindings is accepted and
HOST-PUB-001 publication evidence remains open. D2c.4 contract evidence covers28creation-rooted
...
`CMB-RSH-002` [static Content](../research/combat-content-static-schema-spike.md) and `CMB-RSH-003`
[mutable state](../research/combat-mutable-state-spike.md), plus `CMB-RSH-004`
...
handoffs. `CMB-DES-004` [cost/resolution ordering](../design/combat-cost-resolution-order-v1.md)
is complete for review: atomic commitment/costs, role-ordered results and simultaneous-stage
extension boundaries. `CMB-DES-005` [settlement/disclosure](../design/combat-settlement-disclosure-v1.md)
...
defines 25 staged tasks and evidence requirements. [Review4](../reviews/combat-cycle-plan-review-4.md)
...
on2026-09-06. [TASK-001 source evidence](../research/combat-source-freeze-v1.md) is complete in
`635d95a`: 357 defined loss values preserved and three missing defender +2 cells assigned10% by
accepted `CMB-SRC-RUL-001`. Research supports the amendment; historical intent remains unproved.
Calendar/break-off findings constrain subsequent contracts and future maturity execution remains
explicitly gated. [TASK-002 Content7 contract](../specs/combat-content-v7.md) is frozen in `c465a0f`,
with canonical bytes/hash,70 rejection vectors and eight initial custody/escape geometry probes.
[Checkpoint A author check](../reviews/combat-checkpoint-a-author-check.md) is recorded at `4303004`;
[TASK-003A Setup/initial-ledger contract](../specs/combat-creation-ledger-v1.md) is frozen at `23c3fff`
with63 rejection vectors. Parent003 is split into bounded A–D slices;003B durable World/settlement
contracts are complete as a contract slice. [Review5](../reviews/combat-progress-review-5.md) covers
progress through003B: Ready with non-blocking follow-ups, status correction applied.
...
private authority fragment with author checks. Subsequent bounded contract packets reach Reaction
completion after second movement through `003D2c.3q`; D2c.4 integrates the selected authority and
closes parent003D2c/003. The combined plan's current status table is authoritative for that detailed
chain. Task004, checkpoint B, and all production implementation remain gated.
...
actionable findings (report retained locally). Owner then authorized [review9](../reviews/combat-progress-review-9.md)
for the full unmerged checkpoint throughD2b.1: Ready with non-blocking follow-ups; its sole status
finding is corrected. [Review10](../reviews/combat-progress-review-10.md) returned Ready with
non-blocking follow-ups for D2b.2/D2c.1; both findings are corrected. Owner then authorized review11
acrossPR95–98 throughD2c.2a: Ready, no findings. Owner subsequently authorized review12 for
Reserve/opening plus Movement preparation at `01ded95` and maintenance at `0e815b2`: Ready,
...
author verification only. Owner later authorized [review15](../reviews/combat-inherited-reserve-release-review-15.md) for3i; it returned Ready with a
non-blocking retained-coverage finding corrected before `719ea0d`. The15of15 sequence is exhausted;
another review requires explicit owner authorization. Remaining contract gates stay open.
...
| --- | --- | --- | --- |
| `MVP-GATE-01` scenario-surface freeze | Exact six-turn sequence positions, decisions, rules/tables, map/content records, setup, arrivals, supply/repair, termination, victory, source/rights status, and known rulings | Requirement-to-source matrix; unsupported-item count is explicit; implementation task graph and size review accepted | Scenario content and remaining Land implementation |
| `MVP-GATE-02` content admission | Versioned `Graziani's Offensive` content with stable IDs/hash and no dangling references; copyrighted assets excluded or permitted | Canonical readback, reference validation, independent setup cross-check, provenance/rights audit, historical compatibility | Scenario execution |
| `MVP-GATE-03` six-turn authority | Every mechanic and victory rule reachable in the admitted scenario executes through Umpire commands/events with deterministic fallback and explicit unsupported rejection | Focused branch tests, replay cuts, all reachable table coordinates, obligation/conservation checks, no silent approximation | Campaign lifecycle and UI completion |
...
identity, or authoritative meaning requires a separate contract and recovery plan; it cannot be
hidden inside a milestone implementation.
