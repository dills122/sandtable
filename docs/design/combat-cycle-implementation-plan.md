...
| `008B` preamble/sequence adapter, parent | B1 and B2 below | Opening and stage event parity, prior-prefix and old-reader negatives | Open; complete only after both children | No |
...
| `008C` Weather adapter | B1 | Retained dice/RNG and source/context parity across replay cuts through state6 | Pending; needs accepted opening history | No |
| `008B2` stage-entry adapter | C | Four explicit-none stage events from fully replayed Weather through state10 Reserve; all Weather kinds, nine receipts and null-side order handoff | Pending; requires real Weather replay | No |
| `008D` Reserve adapter | B2 | Designation/completion receipts, own history and no resource reset | Pending; needs complete stage-entry history | No |
