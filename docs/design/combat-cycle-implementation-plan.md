...
Merge snapshot: remote main `dd22088` (reconciliation PR135), 2026-09-20 UTC. All accepted slices through F1 are included; exact main tree equals reviewed F1 checkpoint `32e4e6d`.
...
| Slice / objective | After | Acceptance evidence and test focus | Current status / blocker | On remote main |
| --- | --- | --- | --- | --- |
...
| `008A1c` creation request and Created11 binding | A1b | Exact request/Created11 goldens, identity/retry conflicts, changed trusted artifact and raw-byte negatives | Implemented and reviewed on `codex/combat-task008-creation-binding`; focused52/full1,840 pass, three independent rounds complete | Yes, `dd22088` (#135) |
| `008A2` Snapshot12 creation root/readback | A1c | Exact creation Snapshot12 golden; recompute from trusted request/Created11/World; disabled-admission readback and noninitial rejection | Implemented and reviewed on `codex/combat-task008-creation-snapshot`; focused34/full1,874 pass, three Ready rounds; publication evidence boundary pinned below | Yes, `dd22088` (#135) |
...
| `008B1` opening preamble | A2 | Six frozen creation-rooted traces through four opening events/state5 Weather entry; exact bytes at every cut, actor/retry/forgery and old-reader negatives | Implemented and reviewed on `codex/combat-task008-opening-preamble`; focused12/full1,886 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008C` Weather adapter | B1 | Retained dice/RNG and source/context parity across replay cuts through state6 | Implemented and reviewed on `codex/combat-task008-weather`; focused42/full1,928 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008B2` stage-entry adapter | C | Four explicit-none stage events from fully replayed Weather through state10 Reserve; all Weather kinds, nine receipts and null-side order handoff | Implemented and reviewed on `codex/combat-task008-stage-entry`; focused17/full1,945 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008D` Reserve codec/adapter parent | D1 and D2 below | Designation/completion bytes, own history and no resource reset; terminal projection requires019A | D1/D2 codecs and019A terminal replay implemented, reviewed and included in reconciled main | Yes, `dd22088` (#135) |
| `008D1` Reserve designation | B2 | Full state10 predecessor, optional own none→I designation, exact pre-completion bytes/history and unchanged resources | Implemented and reviewed on `codex/combat-task008-reserve-designation`; focused23/full1,968 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008D2` Reserve completion codec | D1 | History-derived OpeningBase and exact completion2 bytes including ordinal1 identity; no fabricated terminal state | Implemented and reviewed on `codex/combat-task008-reserve-completion`; focused44/full1,989 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `019A` first-cycle opening projector | D2 | Apply same completion2 event atomically; terminal Reserve state11/12, complete retry/readback and Movement handoff | Implemented and reviewed on `codex/combat-task019a-first-opening`; focused67/full2,012 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008E` Movement adapter, parent | E1 and E2/G1 below | Move/stop/completion event parity, CP/DP/history and old-context rejection | E1 and E2/G1 implemented and reviewed; ordinary Movement adapter scope complete | Yes, `dd22088` (#135) |
| `008E1` ordinary Move4 | D and019A | Actual completed Reserve history; both owners, seven moves each, CP/DP/route/progress and strict replay | Implemented and reviewed on `codex/combat-task008-inherited-movement`; focused31/full2,020 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008E2/G1` route lifecycle | E1 | Owner stop2, System empty-cohort resolution2, owner Movement completion3 and actual end proof | Implemented and reviewed on `codex/combat-task008-movement-lifecycle`; focused20/full2,032 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008F` Reaction adapter, parent | F1–F6 below | Selected trigger/participant/closure histories, position/owner privacy and unsupported-profile rejection | Selected F1–F6 implemented and reviewed; full restore remains008H | No |
...
| `008F5` second reactor move | F2 first-participant-move cut | Actual second move with route/history/receipt continuity | Implemented and reviewed in main-based PR136; focused22/full2,093 pass, three Ready rounds | No |
| `008F6` second-move completion | F5 | Participant completion, empty stop resolution and window closure | Implemented and reviewed in main-based PR136; focused24/full2,104 pass, three Ready rounds | No |
| `008G` Breakdown adapter, parent | E2/G1 and G2 below | Stop/resolution/completion, BP/lot provenance and Combat-entry boundary | Assigned E2/G1 and G2 evidence complete for empty-cohort profile; positive vehicles unsupported | Yes, `dd22088` (#135) |
| `008G2` Breakdown completion | E2/G1 | System completion from actual end proof into Combat entry; preserve World/RNG and receipts | Implemented and reviewed on `codex/combat-task008-breakdown-completion`; focused24/full2,044 pass, three Ready rounds | Yes, `dd22088` (#135) |
| `008H` noninitial Snapshot12 and full restore | B–G and `019A` first opening | Complete trusted event/receipt ledger, each causal cut and tamper/reorder/omission rejection; recover with fresh admission disabled | H0 contract accepted; runtime routing and disabled-admission restore remain pending | No |
| `008H0` literal inherited Snapshot12 contract | B–G/019A selected cuts | Preserve root order and creation bytes; typed inherited arms, every-cut full-root vectors and equal shared-cut bytes | Contract implemented;368cuts/286histories/62sharedgroups,15predecessor oracles and dev+three Ready reviews pass; no runtime restore claim | No |
...
**F execution refinement.** [Trigger](../specs/combat-inherited-reaction-trigger-v1.md) consumes exactly the first E1 move, then commits the actual return Move4 opening one Reaction opportunity. Execute **F1 → F2**, with [direct closure F3](../specs/combat-inherited-reaction-closure-v1.md) branching from F1. [Active fallback F4](../specs/combat-inherited-reaction-active-fallback-v1.md) and [second move F5](../specs/combat-inherited-reaction-second-move-v1.md) branch from F2's first-participant-move cut; [completion F6](../specs/combat-inherited-reaction-movement-completion-v1.md) follows F5. [F2 lifecycle](../specs/combat-inherited-reaction-lifecycle-v1.md) supplies the actual participant/stop/closure sequence. G2-before-F1 is execution order, not a fabricated G2-terminal dependency. F3–F6 are required by the selected Reaction traces in [authority composition](../specs/combat-authority-composition-v1.md#selected-profile-and-trace-set); F1/F2 alone do not close F. Each child retains the five-primary-file limit; split by causal event if needed. Persisted window/opportunity identities remain distinct from later public handles. Positive vehicles and multiple opportunities remain unsupported.

**Initial H boundary.** Restore every retained cut of implemented B–G/019A families with fresh admission disabled. Checkpoint D explicitly covers states whose handlers exist; Tasks009–019 extend the same restore contract and cumulative 28-trace target as their authority becomes available. Requiring later Reserve/repeat authority before the initial H dependency gate would invert Task009's dependency on005–008. Keep every selected trace and its later owner visible; do not claim all28 runtime traces, parent publication closure or completed later gameplay from initial H. HOST-PUB-001 remains open independently.

**H execution refinement.** [Literal inherited Snapshot12](../specs/combat-inherited-snapshot-v1.md) supplies additive H0 contract evidence before runtime routing. Preserve the existing nineteen-field root and exact creation bytes; typed Reserve/inherited-cycle arms retain actual membership, route, progress and interruption evidence. Equal trusted histories must yield equal root bytes across family interfaces. H1 covers pre-cycle routing, H2 ordinary Movement/Breakdown, H3 Reaction, and H4 the full-root codec with actual fresh-admission-disabled restore. Split a runtime child further if needed to retain bounded review scope; H5 is reserved for cumulative proof if H4 cannot contain it. Each child requires dev review and three independent rounds. Contract vectors alone do not close H, and later Tasks009–019 must extend reachable composition without discarding inherited evidence.
...
another subsystem, split and re-scope before edits. Task016 is accepted at `d480556` after full2,331/Boundary81, clean build/format, development review, three fresh Ready reviews (third independently rebuilt full solution), and exact-candidate CI. [Acceptance evidence](../../.planning/combat-task008-delivery/task016-evidence.md) retains checks and limits. Bounded dormant checkpoint G settlement is complete; actual campaign/public integration remains open.
...
isolated mechanism evidence from actual campaign provenance.
