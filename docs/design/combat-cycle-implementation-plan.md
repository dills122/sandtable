...
`G0` means recorded POL-001–008 decisions, owner plan review and the outstanding independent
combined-plan review. TASK-001–004 produce source/contract artifacts; runtime consumers start only
...
| Task / size | Output and acceptance criteria | Verification | Dependencies / likely paths |
| --- | --- | --- | --- |
| `CMB-TASK-003` / M, 3–5 files | Freeze CON-002–004 world/event/snapshot/command schemas and state transitions. Allocate exact versions after registry inventory; pin causal receipts, bounds, loss/guard/CP conservation, cycle prefix, deadline and migration/recovery behavior. | Field-by-field design trace, canonical positive/negative vectors and restart-cut matrix; explicit unsupported-state rejection. | 001–002; new authority contract packet and vectors under [design](./), [Campaign tests](../../tests/Cna.Core.Tests/Campaigns/). |
...
| `CMB-TASK-018` / M, 3–5 files | Implement retained Movement-end proximity and immediate next-Movement exception. Repeated Movement preserves CP/BP/bands/broken lots, ammo/TOE/Cohesion; offensive-use history is not reset. | `CycleMovement` unused/skipped exception expiry, changed enemy position, mandatory overspend, retained Breakdown obligations and exhausted assault rejection. | 017; [Campaigns](../../src/Cna.Core/Campaigns/), focused Core tests. |
| `CMB-TASK-019` / M, 3–5 files | Implement first opening, semantic progress, supported continuation witness and repeat/finish control. Prefix excludes opening event; finish enters same-slot Truck Convoy without stage housekeeping. | `CycleControl` full truth table, ordinal/prefix forks, no-op/cancel/retain-only history, lost reply/deadline, pending and future obligations; unsupported continuation is not “none legal.” | 018; [Campaigns](../../src/Cna.Core/Campaigns/), [Rules](../../src/Cna.Core/Rules/), focused Core tests. |
...
[Review3](../reviews/combat-settlement-review-3.md) is Ready for DES-004/005 only; neither this plan
nor CYCLE-DES-001 has the outstanding independent assessment. Existing budget remains **3/3 used**.
