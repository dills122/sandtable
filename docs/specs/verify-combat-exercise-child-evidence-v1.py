...
def admit_manifest(manifest,source):
    ...
    require(manifest['controller']==expected['controller'])
    require(manifest['request']['schedule']['roleOrder']==expected['request']['schedule']['roleOrder'])
    end=len(b.decode_reference(source)['events'])
    require(manifest['request']['maximumAcceptedTransitions']<=end or b.satisfies(b.checkpoint(source,end),manifest['request']['requestedTerminal']))
        ...
        try:
            ...
            if actor!='system':
                ...
                if fault['kind']=='controller-invalid' and cut==fault['atTransition']:
                    ...
                    attempt=dict(atTransition=cut,initiator=actor,trustedClock=copy.deepcopy(clock),sideProposalJson=public_raw(source,bad,'Submission').decode(),outcomeJson=public_raw(source,rejected_outcome,'Outcome').decode())
...
def representative_sources():
    rows=b.source_cases();selected=[]
    predicates=[lambda x:x['reference']['surface']=='corrected-v3',lambda x:x['sourceId'].endswith('.fallback-retreat'),lambda x:x['sourceId'].endswith('.fallback-custody'),
        lambda x:x['sourceId']=='inherited-reserve.axis',lambda x:x['sourceId']=='inherited-control.axis.repeat',lambda x:x['sourceId']=='inherited-control.axis.finish',lambda x:x['reference']['surface']=='historical-v3']
    for predicate in predicates:selected.append(next(x for x in rows if predicate(x)))
    return selected
def test_supported_admission():
    rows=b.source_cases()
    for source in rows:
        manifest=manifest_for(source);assert admit_manifest(manifest,source)==manifest
    source=rows[0];manifest=manifest_for(source)
    for field in ('controller','roleOrder'):
        mutant=copy.deepcopy(manifest)
        if field=='controller':mutant['controller']['custodyChoice']='escape' if mutant['controller']['custodyChoice']=='guard' else 'guard'
        else:mutant['request']['schedule']['roleOrder']='defender-first' if mutant['request']['schedule']['roleOrder']=='attacker-first' else 'attacker-first'
        rejected(lambda:admit_manifest(mutant,source));child=run(mutant,source)
        assert child['result']['failure']=='admission-rejected' and not child['steps'] and not child['checks'] and child['reconstruction'] is None and child['readjudication'] is None
    incomplete=next(x for x in rows if x['reference']['surface']=='fallback-v2');overrun=manifest_for(incomplete);overrun['request']['maximumAcceptedTransitions']+=1
    rejected(lambda:admit_manifest(overrun,incomplete));assert run(overrun,incomplete)['result']['failure']=='admission-rejected'
    headroom=copy.deepcopy(manifest);end=len(b.decode_reference(source)['events']);headroom['request']['maximumAcceptedTransitions']=end+1
    assert admit_manifest(headroom,source)==headroom;child=run(headroom,source);assert child['result']['status']=='succeeded' and len(child['steps'])==end
    return dict(canonicalSources=len(rows),unsupportedConfigurations=2,incompleteOverrunRejected=1,completeHeadroomAdmitted=1)
def test_representatives():
    for source in representative_sources():
        child=run(manifest_for(source),source);RUNS[source['sourceId']]=child
        assert child['result']['failure']==('step-limit-exceeded' if source['reference']['surface']=='fallback-v2' else None)
        assert validate_child(raw(child,'Child'),source,child['manifest'])==child
    return len(representative_sources())
def test_boundary():
...
def test_failures_and_forgery():
    ...
    for kind,at,failure in (('cancel',0,'cancelled'),('controller-invalid',1,'execution-rejected'),('reconstruction',0,'reconstruction-failed'),('readjudication',0,'readjudication-failed')):
        ...
        if kind=='controller-invalid':
            attempt=child['failedAttempt'];assert attempt['trustedClock']==m['trustedSchedule'][at] and attempt['trustedClock']['actor']==attempt['initiator']
            assert set(attempt)=={'atTransition','initiator','trustedClock','sideProposalJson','outcomeJson'}
            original=copy.deepcopy(m['trustedSchedule'][at]);attempt['trustedClock']['admittedAt']=None
            assert m['trustedSchedule'][at]==original and child['manifest']['trustedSchedule'][at]==original
            attempt['trustedClock']=copy.deepcopy(original);checks+=3
...
def generated_fixture():
    ...
    summaries=[]
    for source in b.source_cases():
        b.authenticate_source(source);ref=b.decode_reference(source)
        row=dict(sourceId=source['sourceId'],sourceLineageHash=source['sourceLineageHash'],referenceTranscriptHash=source['referenceTranscriptHash'],retainedTransitions=len(ref['events']),referenceEventHash=stream_hash(INVENTORY['domains']['events'],ref['events']))
        raw(row,'SourceSummary');summaries.append(row)
    names=[x['sourceId'] for x in representative_sources()]+['failure.'+x for x in ('cancel','controller-invalid','reconstruction','readjudication','admission')]
    selected=[dict(name=name,childHash=child_hash(RUNS[name]),childCanonicalJson=raw(RUNS[name],'Child').decode()) for name in names]
    return dict(contractVersion=1,scope='private checkpoint-scoped contract evidence; authenticated reference summaries are not child execution claims; no runtime registration or disk publication',sourcePins=INVENTORY['sourcePins'],summaries=summaries,goldens=selected,isolatedCompletionWitnesses=completion_witnesses())
def test_fixture():
    value=generated_fixture();expected=fixture_bytes(value);require(FIXTURE.read_bytes()==expected);return dict(bytes=len(expected),summaries=len(value['summaries']),goldens=len(value['goldens']))
def main():
    started=time.monotonic()
    for test in (test_boundary,test_supported_admission,test_registry_execution,test_failures_and_forgery,test_identity_and_controller,test_fixture):print(test.__name__,test(),round(time.monotonic()-started,3),flush=True)
    print('B2 PASS 6 groups',round(time.monotonic()-started,3),flush=True)
