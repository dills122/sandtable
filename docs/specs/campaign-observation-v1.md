...
| `OBS-002` | Every accepted observation records observation contract version 1, projection policy ID `sandtable.observation.own-elements-only.v1`, campaign ID, state version, canonical ruleset hash, public scenario ID, and viewer side. Exact Content Pack schema, format, ID, ruleset ID, and hash remain server-side and never appear in an observation. |
...
| `OBS-006` | No opponent element ID, formation ID, organization ID, capability value, occupancy association, force count, aggregate, contact, dummy, placeholder, or other value derived from opposing force state appears in the observation object or canonical bytes. A location ID may independently appear in public topology; it must never be associated with an opposing element or occupancy fact. Version 1 represents opponent knowledge as absence, not an empty contact whose shape leaks a count or type. |
