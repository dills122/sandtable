...
| `EXR-025` | At each nonterminal step the runner queries `system`, `axis`, and `commonwealth` in that fixed order and requires all queries to succeed. Ordinary policies require exactly one audience with a nonempty set; zero is `NoUniqueLegalAction` and multiple is `InvariantFailed`. Explicit ZOR `reaction-*` policies additionally admit exactly one reacting player alongside the exact System unavailable/timeout pair, or a sole System no-eligible-reactor close. The player set contains only Reaction moves and exactly one decline or participant completion. Named System policy selects its reason-specific close when applicable; otherwise the reacting policy selects one current player action. Malformed Reaction shapes fail closed. This bounded arbitration does not add simultaneous-action scheduling. |
...
| `EXR-027` | Movement policies may retain only Runner-local accepted own-element IDs needed for bounded selection; explicit Reaction policies additionally retain accepted move counts for the current episode and completion counts for the current window. Every selected move and completion must be a current observation-derived public candidate submitted through the ordinary membership-checked path. Successful Movement evidence must bind exact event, Reserve, move, CP/ledger, terminal, reconstruction, and re-adjudication semantics; aggregate counts or fingerprints alone are insufficient. |
...
| `EXR-AC-014` | Query all three audiences at each current nonterminal checkpoint, plus test-only zero-active and multi-active fixtures | Query order is fixed; ordinary policies require exactly one active audience, with zero failing `NoUniqueLegalAction` and multiple failing the named invariant. Explicit Reaction policies accept only the bounded shape in `EXR-025`; missing, mixed, or extra kinds fail closed, and valid player/System paths select exactly one current action. |
